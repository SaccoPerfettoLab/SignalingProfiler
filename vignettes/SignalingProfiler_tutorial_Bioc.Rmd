...
-   [**logpval**]{style="color:orange"}: the -Log(p-value) associated to your fold-change;
...
-   [**logpval**]{style="color:orange"}: the -Log(p-value) associated to the statistical test assessing the significance of your comparisons;
...
The information about how phosphosites regulate the protein they are on is retrieved from SIGNOR and PsP databases (`good_phos_df_human_act`, `good_phos_df_human_act_aapos`). SignalingProfiler updates its regulons (regulatory networks) twice a year. If the user want to use updated information it can create its own information about regulatory phosphosites. Importantly, if you want to include PsP information you have to manually download from PsP database the Regulatory_sites and Kinase_Substrates file.
...
::: {style="background-color: #f9f9f9; border-left: 5px solid #3c763d; padding: 1em; margin: 1em 0 1em 2em;"}
**Built-in objects** 
The information is stored in different built-in objects that can contain all interactions `PKN_human_ind` or only direct interactions (`PKN_human_dir`).
:::
