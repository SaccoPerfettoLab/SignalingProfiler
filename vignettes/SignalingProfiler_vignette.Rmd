...
inv_optimized_graph_rds <- readRDS('./inv_optimized_network.sif')
inv_optimized_graph_sif <- readr::read_tsv('./inv_optimized_network.RDS',
                                       col_names = c('source', 'interaction', 'target'))
...
Once you have your context-specific model, you can *in silico* validate it, computing the modulation of activity of relevant phenotypes for your context. We call this score **phenoSCORE**. It is very useful, especially when you have some expectations about your model characteristics. 
For example, if you are treating tumor cells you may expect the APOPTOSIS phenotype to be activated.

We compute the **phenoSCORE** using:

-   the nodes in the context-specific model;
-   paths table from SIGNOR proteins to SIGNOR phenotypes containing a different significant causal paths, derived from our in-house recently published resource [ProxPath](https://www.biorxiv.org/content/10.1101/2023.01.09.523265v1.full.pdf); it is available as built-in object `paths_to_pheno`

### Choose the nodes to connect to the phenotype
You can choose the nodes in your model to connect to the phenotype with the `use_carnival_activity` parameter.
- `use_carnival_activity = FALSE`: exploits the nodes derived from experimental data;
- `use_carnival_activity = TRUE`: exploits all the nodes in the final model; 

### ProxPath customization
Briefly, **ProxPath** reports the regulators of phenotypes.
Regulators are defined as proteins connected to the phenotype with a maximum
of 4-steps-length path. The user can choose closer regulators to the phenotype
modulating the length of the path with the `max_path_length` parameter (default is 4).
...
### Cascade removal
If two phenotype regulators are in the same linear cascade in the network, 
`remove_cascade = TRUE` allows to consider only the regulator
at the end of the signaling cascade. 

### Output customization
The `compact = TRUE` paramter allow to see the score of each phenotype.
If you want to see each regulator of the phenotype, set `compact = FALSE`.
...
compute_phenoscore(sp_output = toy_sp_output, 
                   desired_phenotypes = 'G2/M_TRANSITION', 
                   max_path_length = 4, 
                   remove_cascade = TRUE,
                   compact = TRUE)
```
