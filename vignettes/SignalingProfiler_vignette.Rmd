...
Here, we create what we call a **naive network**, a graph connecting your proteins through ALL the possible SHORTEST causal paths of a user-specified length (1 to 4). We don't consider the causality of the edges, just distance. As such, in this naive network there may be two paths with same length, but different causal meaning.

In this step, you are building the skeleton of the signal in your cellular model, looking across all the possible routes available in SIGNOR and PsP databases. These databases are available as built-in object in the package, in tabular and igraph object format, and for both *human* and *mouse* organisms.
...
You can modulate the structure of the naive network:
...
```{r eval = TRUE}
#
one_layer_toy <- one_layer <- one_layer_naive_network(starts_gn = c('Flt3'),
